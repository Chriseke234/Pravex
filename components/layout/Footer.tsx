...
export function Footer() {
  return (
    <footer className="border-t border-slate-200/80 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950 text-slate-600 dark:text-slate-400 text-sm transition-colors duration-300">
      {/* Trust Badges Banner */}
      <div className="border-b border-slate-200/60 dark:border-slate-800/60 bg-slate-100/60 dark:bg-slate-900/40 py-6">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-10">
        ...
        <div className="space-y-4 lg:col-span-1">
          ...
          <div className="pt-2 flex items-center gap-3 text-slate-500 text-xs">
            ...
            <span>•</span>
            <Link href="/faqs" className="hover:text-amber-500 transition-colors">FAQs</Link>
          </div>
        </div>
        ...
        {footerSections.map((section) => (
          <div key={section.title} className="space-y-4">
            <h3 className="text-xs font-bold text-slate-900 dark:text-slate-200 uppercase tracking-wider">{section.title}</h3>
            <ul className="space-y-2.5">
              {section.links.map((link) => (
                <li key={link.name}>
                  <Link
                    href={link.href}
                    className="text-xs text-slate-600 dark:text-slate-400 hover:text-amber-600 dark:hover:text-amber-400 transition-colors block py-0.5"
                  >
                    {link.name}
                  </Link>
                </li>
      ...
      <div className="border-t border-slate-200/80 dark:border-slate-800/80 bg-white/50 dark:bg-slate-950/80">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-4 text-xs text-slate-500 leading-relaxed">
          ...
          <div className="pt-4 border-t border-slate-200 dark:border-slate-800/60 flex flex-col sm:flex-row items-center justify-between gap-4 text-slate-500 text-[11px]">
            ...
            <div className="flex items-center gap-6">
              ...
              <Link href="/cookies" className="hover:text-amber-500 transition-colors">Cookie Preferences</Link>
            </div>
          </div>
        </div>
      </div>
