...
function ThemeToggle({ className }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={cn(
        "p-2 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800/80 border border-slate-800/60 transition-colors flex items-center justify-center",
        className
      )}
      aria-label="Toggle theme"
    >
      <Sun className="w-4 h-4 hidden dark:block text-amber-400" />
      <Moon className="w-4 h-4 block dark:hidden text-amber-400" />
    </button>
  );
}
...
export function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="lg:hidden flex items-center gap-2">
          ...
          <button
            ...
            className="w-10 h-10 rounded-xl bg-slate-900 border border-slate-800/80 flex items-center justify-center text-slate-300 hover:text-white hover:border-amber-500/40 transition-colors focus:outline-none"
          >
            {isMobileMenuOpen ? (
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 top-16 z-50 lg:hidden flex flex-col animate-fade-in">
          {/* Backdrop Blur Overlay */}
          <div
            className="fixed inset-0 top-16 bg-slate-950/80 backdrop-blur-md"
            onClick={closeMenu}
            aria-hidden="true"
          />

          {/* Slide-in Menu Panel */}
          <div className="relative ml-auto w-full max-w-md h-[calc(100vh-4rem)] bg-slate-950/98 backdrop-blur-2xl border-l border-slate-800/90 shadow-2xl flex flex-col justify-between overflow-hidden z-10 animate-slide-in">
                ...
                {user ? (
                  ...
                  <div className="space-y-2.5">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">Client Access</span>
                    </div>
                ...
                {MEGA_MENUS.map((menu) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[10px] text-slate-400 font-medium">
                              {menu.items.length} services available
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-[9px] font-semibold text-slate-400 bg-slate-800/80 border border-slate-700 px-2 py-0.5 rounded-full">
                            {menu.badge}
                          </span>
                          <ChevronDown
                            className={cn(
                      ...
                      {/* Accordion Content */}
                      {isExpanded && (
                        <div className="px-3 pb-3 pt-1 space-y-1 border-t border-slate-800/80 bg-slate-950/50">
                          {menu.items.map((sub) => {
                            const SubIcon = sub.icon;
                            ...
                            return (
                              <Link
                                ...
                                onClick={closeMenu}
                                className={cn(
                                  "flex items-center gap-3 p-2.5 rounded-xl text-xs font-medium transition-colors group",
                                  isSubActive
                                    ? "bg-amber-500/10 text-amber-400 font-semibold"
                                    : "text-slate-300 hover:text-white hover:bg-slate-800/60"
                                )}
                              >
                                <div className="w-6 h-6 rounded-lg bg-slate-800/80 flex items-center justify-center text-slate-400 group-hover:text-amber-400 shrink-0">
              ...
              <div className="space-y-1 pt-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <Link
                    href="/about"
                    onClick={closeMenu}
                    className="flex items-center gap-2.5 p-3 rounded-xl bg-slate-900/50 border border-slate-800/70 text-xs font-semibold text-slate-300 hover:text-white hover:bg-slate-900 transition-colors"
                  >
                  ...
                  <Link
                    ...
                    className="flex items-center gap-2.5 p-3 rounded-xl bg-slate-900/50 border border-slate-800/70 text-xs font-semibold text-slate-300 hover:text-white hover:bg-slate-900 transition-colors"
                  >
                    <Mail className="w-4 h-4 text-amber-400" /> Contact Support
                  </Link>
                  <Link
                    ...
                    className="flex items-center gap-2.5 p-3 rounded-xl bg-slate-900/50 border border-slate-800/70 text-xs font-semibold text-slate-300 hover:text-white hover:bg-slate-900 transition-colors"
                  >
                    <Calculator className="w-4 h-4 text-amber-400" /> Loan Calc
                  </Link>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-amber-500/10 via-amber-500/5 to-transparent border border-amber-500/20 rounded-2xl p-3.5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400">24/7 Priority Member Line</p>
                  </div>
                </div>
            ...
            <div className="p-4 border-t border-slate-800/80 bg-slate-950/95 shrink-0 flex items-center justify-between text-[11px] text-slate-400">
              <div className="flex items-center gap-2">
